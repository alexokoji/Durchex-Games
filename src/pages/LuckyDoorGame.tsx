...
export default function LuckyDoorGamePage() {
  ...
  const playGame = async () => {
    ...
    try {
      ...
      const bet = await wallet.placeBet({ gameId: 'luckydoor', gameName: 'Lucky Door', stake });
      if (bet)
        ...
        <Card sx={{ background: darkCard, border: `1px solid ${darkBorder}`, borderRadius: 2, p: 3 }}>
          ...
          {gameState.gameOver && (
            <Box sx={{ p: 2, background: '#1a1a2e', borderRadius: 1, textAlign: 'center', mb: 2 }}>
              ...
              >
                {gameState.won ? '✓ Right Door!' : '✗ Wrong Door'}
              </Typography>
            </Box>
          )}
          {gameState.gameOver && (
