...
import { DragonTowerGame } from '../games/dragonTower/DragonTowerGame';
import GamePageWrapper from '../components/games/GamePageWrapper';
import { playSound } from '../constants/gameAssets';

export default function DragonTowerGamePage() {
  const wallet = useWallet();
  const { user } = useAuth();
  const toasts = useToasts();
  const [stake, setStake] = useState(100);
  const [gameState, setGameState] = useState({ gameOver: false, level: 0, won: false });
  const [loading, setLoading] = useState(false);
  const [_engine] = useState(() => new GameEngine());
  const [game] = useState(() => new DragonTowerGame());
  ...
  const playGame = async () => {
    ...
    try {
      const result = await game.play({ gameId: 'dragontower', userId: user.id, sessionId: `dt_${Date.now()}`, stake, currency: wallet.currency, autoPlayCount: 0, mode: 'single' });
      setGameState({ gameOver: true, level: (result.details?.level as number) ?? 0, won: result.won });
    ...
    <GamePageWrapper gameId="dragontower">
      ...
      <Card sx={{ background: darkCard, border: `1px solid ${darkBorder}`, borderRadius: 2, p: 3 }}>
        ...
        <Button fullWidth variant="contained" disabled={loading || gameState.gameOver} onClick={playGame} sx={{ background: neonGreen, color: '#000', fontWeight: 900, py: 1.5, mb: 2 }}>
          {loading ? 'Climbing...' : 'Climb Tower'}
        </Button>

        {gameState.gameOver && (
          <Box sx={{ p: 2, background: '#1a1a2e', borderRadius: 1, textAlign: 'center', mb: 2 }}>
            <Typography sx={{ fontSize: '1.1rem', fontWeight: 800, color: gameState.won ? neonGreen : '#ff6b7a' }}>{gameState.won ? '🎉 Won!' : '❌ Lost'}</Typography>
          </Box>
        )}

        {gameState.gameOver && <Button fullWidth variant="outlined" onClick={() => setGameState({ gameOver: false, level: 0, won: false })} sx={{ borderColor: neonGold, color: neonGold }}>Play Again</Button>}
      </Card>
