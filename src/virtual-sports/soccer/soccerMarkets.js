...
exports.computeExpectedGoals = computeExpectedGoals;
exports.computeEventRates = computeEventRates;
exports.buildScoreGrid = buildScoreGrid;
exports.buildSoccerMarkets = buildSoccerMarkets;
 ...
 */
function computeExpectedGoals(home, away) {
    const hStrength = (0, teamDatabase_1.teamStrength)(home);
    const aStrength = (0, teamDatabase_1.teamStrength)(away);
    ...
    const hp = (0, teamDatabase_1.getPersonality)(home);
    const ap = (0, teamDatabase_1.getPersonality)(away);
...
function computeEventRates(home, away) {
    const hp = (0, teamDatabase_1.getPersonality)(home);
    const ap = (0, teamDatabase_1.getPersonality)(away);
...
function sumGrid(grid, predicate) {
    let s = 0;
    for (let h = 0; h <= MAX_SCORE; h++) {
        for (let a = 0; a <= MAX_SCORE; a++) {
            if (predicate(h, a))
...
function buildSoccerMarkets(matchId, home, away, overround = oddsEngine_1.DEFAULT_OVERROUND) {
    ...
    for (let h = 0; h <= 4; h++)
        for (let a = 0; a <= 4; a++)
            scoreEntries.push({ h, a, p: grid[h][a] });
    ...
    for (const side of ['home', 'away']) {
        ...
        for (const line of [0.5, 1.5, 2.5]) {
            ...
            markets.push(makeMarket(matchId, `tt-${side}-${line}`, 'TEAM_TOTAL', `${teamRef.shortName} — Over/Under ${line}`, makeOptions([
