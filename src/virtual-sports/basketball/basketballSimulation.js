...
exports.simulateBasketballMatch = simulateBasketballMatch;
exports.resolveBasketballSelection = resolveBasketballSelection;
exports.basketballEventsUpTo = basketballEventsUpTo;
const basketballMarkets_1 = require("./basketballMarkets");
...
function randPlayer(rand) {
    return PLAYERS[Math.floor(rand() * PLAYERS.length)];
}
function sampleNormal(rand, mean, std) {
    const u1 = Math.max(0.0001, rand());
    const u2 = rand();
    const z = Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
    return mean + std * z;
}
function simulateBasketballMatch(home, away, seed) {
    const rand = mulberry32(seed);
    const proj = (0, basketballMarkets_1.projectBasketball)(home, away);
...
function resolveBasketballSelection(selection, match) {
    ...
    switch (marketCategory) {
        ...
        case 'PERIOD_WINNER': {
            ...
            if (optionId === 'home')
                return q1.home > q1.away ? 'win' : 'loss';
            if (optionId === 'away')
                return q1.away > q1.home ? 'win' : 'loss';
            return 'loss';
        }
        case 'HALF_TIME': {
            ...
            if (optionId === 'home')
                return h1 > a1 ? 'win' : 'loss';
            if (optionId === 'away')
                return a1 > h1 ? 'win' : 'loss';
            return 'loss';
        }
        default:
            return 'void';
...
function basketballEventsUpTo(events, gameMinute) {
    return events.filter(e => e.minute <= gameMinute);
}
//# sourceMappingURL=basketballSimulation.js.map