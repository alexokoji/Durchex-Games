...
import { simulateBasketballMatch, resolveBasketballSelection, basketballEventsUpTo, type SimulatedBasketball } from './basketballSimulation';
...
const LIVE_SECONDS    = 120;     // 2 min real-time = full 48 game-minutes
const FINISHED_SECONDS = 25;
const GAME_MINUTES = 48;

export interface BasketballScheduledMatch {
  id: string;
  round: number;
  home: Team;
  away: Team;
  markets: Market[];
  simulation: SimulatedBasketball;
  phase: BasketballPhase;
  gameMinute: number;
  visibleEvents: MatchEvent[];
}

export interface UseBasketballScheduleResult {
  round: number;
  phase: BasketballPhase;
  secondsToNextPhase: number;
  matches: BasketballScheduledMatch[];
  nextRoundIn: number;
  liveCount: number;
}
...
export function useBasketballSchedule({ leagueId, matchesPerRound }: Args): UseBasketballScheduleResult {
  ...
  const buildRound = useCallback((r: number): BasketballScheduledMatch[] => {
    ...
    return pairs.map((p, i) => {
      const matchId = `bbm-${leagueId}-r${r}-${i}`;
      const seed = hash(matchId);
      const simulation = simulateBasketballMatch(p.home, p.away, seed);
      const { markets } = buildBasketballMarkets(matchId, p.home, p.away);
      return {
        id: matchId,
        round: r,
        home: p.home,
        ...
        markets,
        simulation,
        phase: 'betting' as BasketballPhase,
        gameMinute: 0,
        visibleEvents: [],
      };
    });
  ...
  const [matches, setMatches] = useState<BasketballScheduledMatch[]>(() => buildRound(1));
  const [phaseClock, setPhaseClock] = useState({ phase: 'betting' as BasketballPhase, secondsInto: 0 });
  ...
  useEffect(() => {
    setMatches(prev => prev.map(m => {
      ...
      if (phaseClock.phase === 'live') {
        ...
        const gameMinute = progress * GAME_MINUTES;
        return { ...m, phase: 'live', gameMinute, visibleEvents: basketballEventsUpTo(m.simulation.events, gameMinute) };
      }
  ...
  useEffect(() => {
    ...
    const key = `bb-r-${round}-${leagueId}`;
    if (settledRoundsRef.current.has(key)) return;
    settledRoundsRef.current.add(key);
