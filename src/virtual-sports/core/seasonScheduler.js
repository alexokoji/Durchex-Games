...
exports.buildSeasonSchedule = buildSeasonSchedule;
exports.buildLeaguePhaseSchedule = buildLeaguePhaseSchedule;
exports.fixturesForWeek = fixturesForWeek;
 ...
 */
function buildSeasonSchedule(teamIds, seed) {
    if (teamIds.length < 2)
        return [];
    ...
    const secondHalf = firstHalf.map((round, r) => round.map(f => ({
        week: r + 1 + halfRounds,
        homeId: f.awayId,
        awayId: f.homeId,
    })));
 ...
 */
function buildLeaguePhaseSchedule(teamIds, seed, weeks = 8) {
    if (teamIds.length < 4)
        return [];
    const rnd = rngFromSeed(seed);
    const fixtures = [];
    const playedAgainst = new Map();
    for (const t of teamIds)
        playedAgainst.set(t, new Set());
    for (let w = 1; w <= weeks; w++) {
        ...
        for (let i = 0; i < pool.length; i++) {
            const a = pool[i];
            if (usedThisWeek.has(a))
                continue;
            let partner = null;
            for (let j = i + 1; j < pool.length; j++) {
                const b = pool[j];
                if (usedThisWeek.has(b))
                    continue;
                if (playedAgainst.get(a)?.has(b))
                    continue;
                partner = b;
                break;
            }
...
function fixturesForWeek(season, week) {
...
//# sourceMappingURL=seasonScheduler.js.map