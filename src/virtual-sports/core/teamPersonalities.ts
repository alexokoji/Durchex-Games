import type { TeamPersonality, Team } from './types';
 ...
 */
const P: Record<string, TeamPersonality> = {
  // ─── Premier League ────────────────────────────────────────────────────
  mci: { // Manchester City — possession, high press, ruthless finishing
  ...
  liv: { // Liverpool — high press, fast transitions
  ...
  ars: { // Arsenal — build-up play, creative midfield
  ...
  che: { // Chelsea — balanced, set-piece strong
  ...
  mun: { // Manchester United — direct attacks, fast wingers
  ...
  tot: { // Tottenham — pacey, transitions
  ...
  new: { // Newcastle — physical, disciplined
  ...
  whu: { // West Ham — direct, set pieces
  ...
  // ─── La Liga ───────────────────────────────────────────────────────────
  rma: { // Real Madrid — clinical, late-match killer instinct
  ...
  bar: { // Barcelona — tiki-taka, possession dominance
  ...
  atm: { // Atletico Madrid — compact defensive shape, counter
  ...
  sev: { // Sevilla — Europa-style structured
  ...
  rso: { // Real Sociedad — possession + creativity
  ...
  // ─── Bundesliga ────────────────────────────────────────────────────────
  bay: { // Bayern — relentless attack, possession
  ...
  bvb: { // Borussia Dortmund — vertical attacks, pace
  ...
  lev: { // Bayer Leverkusen — possession + flair
  ...
  sge: { // Eintracht Frankfurt — set pieces, physical
  ...
  // ─── Serie A ───────────────────────────────────────────────────────────
  juv: { // Juventus — defensive, structured, counter
  ...
  mil: { // AC Milan — fast wingers, transitions
  ...
  int: { // Inter — Conte-style 3-5-2, wing-back attacks
  ...
  nap: { // Napoli — possession + flair (Spalletti-era identity)
  ...
  // ─── Ligue 1 ───────────────────────────────────────────────────────────
  psg: { // Paris Saint-Germain — star-driven, high attack
  ...
  mar: { // Olympique Marseille — direct, passionate
 ...
 */
export function getPersonality(team: Team): TeamPersonality {
  if (team.personality) return team.personality;
  const explicit = P[team.id];
  if (explicit) return explicit;
  return derivePersonality(team);
}

function derivePersonality(team: Team): TeamPersonality {
