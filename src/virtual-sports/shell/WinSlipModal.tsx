...
export default function WinSlipModal({ ticket, currency, open, onClose }: WinSlipModalProps) {
  ...
  const capture = useCallback(async (): Promise<string | null> => {
    ...
    return toPng(slipRef.current, {
      // 320 px DOM element × pixelRatio 1.5 → 480 px output.
      // 320 px gives the content (payout figure, market labels, header) enough
      // room to render without overflowing — html-to-image doesn't honour
      // overflow:hidden in its foreignObject pass, so the container must be
      // wide enough that no child genuinely overflows.
      pixelRatio: 1.5,
      cacheBust: true,
  ...
  return (
    <Dialog
      ...
      PaperProps={{
        sx: {
          ...
          borderRadius: 3,
          overflow: 'hidden',
          // 10 px side margins on mobile so the card doesn't fill edge-to-edge.
          mx: '10px',
          width: 'calc(100% - 20px)',
          maxHeight: '92vh',
        },
    ...
    >
      ...
      {/* Zero padding so the slip bleeds edge-to-edge inside the dialog.
           overflowY scroll handles many-selection tickets on short screens. */}
      <DialogContent sx={{ p: 0, overflowY: 'auto' }}>
        ...
        >
          ...
          <Box style={{
            ...
            {/* Logo */}
            {logoUrl ? (
              <img src={logoUrl} width={28} height={28} alt="logo"
            ...
            <Box style={{
              ...
              <Typography style={{ fontSize: 8, color: Au, fontWeight: 800, letterSpacing: '0.08em', margin: 0 }}>
                WINNER
              </Typography>
            </Box>
          </Box>
          ...
          <Box style={{
            ...
            {profit > 0 && (
              <Typography style={{
                fontSize: 9, color: 'rgba(0,255,136,0.65)', fontWeight: 700,
            ...
            <Typography style={{
              fontSize: 8, color: 'rgba(255,255,255,0.28)', marginTop: 4, marginBottom: 0,
            }}>
          ...
          {ticket.mode !== 'single' && (
            <Box style={{
              ...
              <Box style={{
                padding: '2px 10px', borderRadius: 10, flexShrink: 0,
          ...
          {/* ── Selections list ── */}
          <Box style={{ padding: '0 9px 9px', overflow: 'hidden' }}>
            {ticket.selections.map((s, i) => {
              ...
              return (
                <Box key={s.id} style={{ overflow: 'hidden' }}>
                  {i > 0 && (
                    <Box style={{ height: 1, background: 'rgba(255,255,255,0.05)', margin: '8px 0' }} />
                  )}
                  <Box style={{ display: 'flex', gap: 8, alignItems: 'flex-start', overflow: 'hidden' }}>
                    {/* Result icon */}
                    ...
                    <Box style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
                      ...
                      <Box style={{ display: 'flex', alignItems: 'center', gap: 6, overflow: 'hidden' }}>
                        <Typography style={{
                        ...
                        {score && (
                          <Typography style={{
                            ...
                            fontVariantNumeric: 'tabular-nums', margin: 0, flexShrink: 0,
                          }}>
          ...
          <Box style={{
            ...
            {ticket.mode !== 'single' && ticket.selections.length > 1 && (
              <Box style={{ textAlign: 'center', minWidth: 0, overflow: 'hidden', flexShrink: 1 }}>
                <Typography style={{ fontSize: 8, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em', margin: '0 0 2px' }}>
                  TOTAL ODDS
                </Typography>
                <Typography style={{
                  fontSize: 13, fontWeight: 800, color: Au,
                  fontVariantNumeric: 'tabular-nums', margin: 0,
                }}>
                  {totalOdds.toFixed(2)}
                </Typography>
            ...
            <Box style={{ textAlign: 'right', minWidth: 0, overflow: 'hidden' }}>
              <Typography style={{ fontSize: 8, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em', margin: '0 0 2px' }}>
                TOTAL PAYOUT
              </Typography>
              <Typography style={{
